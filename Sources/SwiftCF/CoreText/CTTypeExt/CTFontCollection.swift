...
public extension CTFontCollection {
    ...
    ///
    /// This function creates a new collection based on the provided font descriptors. The contents of this
    /// collection is defined by matching the provided descriptors against all available font descriptors.
    ///
    /// - Parameters:
    ///   - queryDescriptors: An array of font descriptors to use for matching. May be NULL, in
    ///   which case the matching descriptors will be NULL.
    ...
    ///
    /// This function returns a retained reference to the array of descriptors to be used to query (match) the
    /// system font database. The return value is undefined if CTFontCollectionCreateFromAvailableFonts
    ...
    ///
    /// This function returns a retained reference to the array of descriptors to be used to query (match) the system font database.
    ...
    /// An array of CTFontDescriptors matching the collection definition or NULL if there are none.
    ///
    /// - Parameter options: The options dictionary. See constant option keys. May be NULL, in
    /// which case this call returns the same results as CTFontCollectionCreateMatchingFontDescriptors,
    /// using the options passed in when the collection was created.
    ...
    ///
    /// An array containing one value for each descriptor. With kCTFontCollectionCopyDefaultOptions, the
    /// values will be in the same order as the results from CTFontCollectionCreateMatchingFontDescriptors
    /// and NULL values will be transformed to kCFNull. When the kCTFontCollectionCopyUnique is set,
    /// duplicate values will be removed. When kCTFontCollectionCopyStandardSort is set, the values will
    /// be sorted in standard UI order.
    /// 
    /// - Parameters:
    ///   - key: The attribute to retrieve for each descriptor in the collection.
    ...
    ///
    /// An array containing one CFDictionary value for each descriptor mapping the requested attribute
    /// names. With kCTFontCollectionCopyDefaultOptions, the values will be in the same order as the
    /// results from CTFontCollectionCreateMatchingFontDescriptors.
    /// When the kCTFontCollectionCopyUnique is set, duplicate values will be removed.
    /// When kCTFontCollectionCopyStandardSort is set, the values will be sorted in standard UI order.
    ///
    /// - Parameters:
    ///   - keys: The attributes to retrieve for each descriptor in the collection.
    ///   - options: Options to alter the return value.
...
private class Context {
    let sortCallback: (CTFontDescriptor, CTFontDescriptor, Any) -> CFComparisonResult
    let context: Any
    init<ContextObject>(sortCallback: @escaping (CTFontDescriptor, CTFontDescriptor, ContextObject) -> CFComparisonResult, context: ContextObject) {
        self.sortCallback = { lhs, rhs, ctx in
            return sortCallback(lhs, rhs, ctx as! ContextObject)
        }
...
extension CTMutableFontCollection {
    ...
    /// - Parameter descriptors: An array of CTFontDescriptorRef. May be NULL to represent an
    /// empty collection, in which case the matching descriptors will also be NULL.
