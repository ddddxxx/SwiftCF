...
extension CTFont: CFCopying {
    ...
    ///
    /// This function provides a mechanism to change attributes quickly on a given font reference in
    /// response to user actions. For instance, the size can be changed in response to a user manipulating a
    /// size slider.
    ///
    /// - Parameters:
    ///   - size: The point size for the font reference. If 0.0 is specified, the original font’s size is preserved.
    ///   - matrix: The transformation matrix for the font. In most cases, set this parameter to be NULL.
